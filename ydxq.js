/*
【MITM】
hostname = planet-api.op.yadea.com.cn
【重写【
填写完整的url https://planet-api.op.yadea.com.cn/user-api/app/user/getUsertoken
*/
...
let ydxq = ($.isNode() ? process.env.ydxq : $.getdata("ydxq")) || "";
let mrArr = [];
let msg = '';
let accountTips = '';
var count=0;
...
async function Envs() {
    if (ydxq) {
        if (ydxq.indexOf("@") != -1) {
            ydxq.split("@").forEach((item) => {
                mrArr.push(item);
            });
        } else if (ydxq.indexOf("\n") != -1) {
            ydxq.split("\n").forEach((item) => {
                mrArr.push(item);
            });
        } else {
